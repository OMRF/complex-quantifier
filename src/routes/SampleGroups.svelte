<script lang="ts">
    ...
    import GroupsBulkProcessingState from './GroupsBulkProcessingState.svelte'
...
<div class="flex flex-col min-h-[16rem]">
    <div class="flex justify-between items-center">
        <h3 class="font-semibold tracking-tight text-3xl">Sample Groups</h3>
        <Button variant="secondary" on:click={addGroup} type='button' disabled={$formData.isBulk}>Add group</Button>
    </div>
    {#if $formData.isBulk}
        <GroupsBulkProcessingState />
    {:else}
                        ...
                        <FormPrimitive.Field {form} name="groups[{i}].name">
                            <Form.Control let:attrs>
                                <input
                                    {...attrs}
                                    type="text"
                                    class="bg-background w-full md:w-1/2 px-5 py-2.5 outline-none border-b md:border-b-0 md:border-r rounded-t-lg md:rounded-r-none md:rounded-l-lg"
                        ...
                        <FormPrimitive.Field {form} name="groups[{i}].name">
                            <Form.Control let:attrs>
                                <input
                                    {...attrs}
                                    type="text"
                                    class="bg-background w-full md:w-1/2 pl-5 pr-5 md:pr-20 py-2.5 outline-none rounded-b-lg md:rounded-l-none md:rounded-r-lg"
                                    placeholder="Column letters separated by commas"
                    ...
                    {#if $errors.groups && $errors.groups[i]}
                        ...
                        <ul class="flex flex-col list-none">
                            {#if $errors.groups[i].name}
                                {@const errors = $errors.groups[i].name}
                            ...
                            {#if $errors.groups[i].columns}
                                {@const errors = $errors.groups[i].columns}
